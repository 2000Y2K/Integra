...
var materialNeurona = new THREE.ShaderMaterial( { 
  ...
  const intensityAmbiente = 0.3;
  const light = new THREE.HemisphereLight(groundColor,skyColor, intensityAmbiente);
  scene.add(light);
  /////
...
function cargarParticulas() // crea tetraedros en puntos random
{
  scene.add(particle);
...
function render(tiempo) // se llama cada frame para renderizar los cambios en la escena
{
  tiempo /= 1000;
  ...
  highlights.forEach((highlight) => // desactiva todos los highlights
  {
  ...
  esferas.forEach((esfera) =>  // renderiza el efecto de llamado en cada esfera
  {
  ...
  if (contenedorInformacion.style.cssText === "visibility: visible;") // detiene movimiento de cerebro y relentiza particulas si esta abierta informacion
  {
    controls.autoRotate = false;
    particle.rotation.y -= 0.001;
  }
  else if ( controls.autoRotate === false) // vuelve al estado normal si no es asi
    {
  ...
  lineas.forEach((linea) => //renderiza el movimiento de las lineas
   {
    ...
    if (intersecados.length && tiempo > 2) // activa el highlight  con hoover y abre la informacion si se clickea
    {
    ...
    highlights.forEach((highlight) =>  // mantiene el highlight activo cuando no hay hoover pero esta clikeado su informacion
    {
...
function iniciar() // carga todos los elementos iniciales
{
  gltfLoader.load('resources/models/Brain.gltf', (gltf) => {
    ...
    // hace que la camara siempre apunte al centro del cerebro
